...
class GameLauncher(QMainWindow):
    def __init__(self):
        ...
        # Data structures
        self.profiles = {}  # name -> dict with keys: exe, resolution, hz, force_hz, default
        ...
        # Load profiles from settings (handle old tuple format)
    ...
    def setup_ui(self):
        ...
        # --- New: Advanced options (Hz and Default) ---
    ...
    def restore_last_selection(self):
        """Restore the last used executable, resolution, Hz, and default flag from settings."""
        if self.current_exe:
    ...
    def on_default_toggled(self, checked):
        ...
        # Optionally disable resolution and Hz controls when default is checked
        res_group = self.findChild(QGroupBox, "")  # Not easy to access by name; we'll use references
        # Instead, we'll directly enable/disable the relevant widgets
        for radio in self.preset_radios:
    ...
    def launch_game(self):
        """Launch the selected executable with the chosen resolution and Hz arguments."""
        ...
        # Otherwise, build command with resolution and Hz arguments
        ...
        try:
            ...
            QMessageBox.information(self, "Launched", f"Game launched with {resolution_str}." +
                                    (f" at {hz} Hz" if self.force_hz_check.isChecked() else ""))
        except Exception as e:
            QMessageBox.critical(self, "Launch Error", f"Failed to launch game:\n{str(e)}")
