...
def main(page: ft.Page):
    ...
    rail = ft.NavigationRail(
        ...
        destinations=[
            ft.NavigationRailDestination(
                icon=ft.icons.DASHBOARD,           # fixed: removed _OUTLINED
                selected_icon=ft.icons.DASHBOARD,
                label="Dashboard",
            ),
            ft.NavigationRailDestination(
                icon=ft.icons.ANALYTICS,           # fixed
                selected_icon=ft.icons.ANALYTICS,
                label="Analytics",
            ),
            ft.NavigationRailDestination(
                icon=ft.icons.SETTINGS,            # fixed
    ...
    page.appbar = ft.AppBar(
        ...
        elevation=0,
        actions=[
            ft.IconButton(icon=ft.icons.NOTIFICATIONS, icon_color=text_primary),   # fixed
            ft.IconButton(icon=ft.icons.PERSON, icon_color=text_primary),          # fixed
        ],
    )
